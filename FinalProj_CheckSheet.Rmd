...
(A) .Rmd Line numbers where at least two different data sources are imported: line 34(first data set: confirmed_cases_worldwide.csv) and line 68(second dataset: confirmed_cases_china_vs_world.csv)


(B) .Rmd Line numbers for inspecting data intake: line 37(head) and line 38(summary)
...
(A) .Rmd Line number(s) for general data wrangling: Filter function has been used many places. I am putting one line where it is used. Line 111(filter), line 95(mutate).


(B) .Rmd Line number(s) for a join operation: line 45(inner join)


(C) .Rmd Line number(s) for a spread or gather operation (or equivalent): line 48 and 49 for spread and gather.


(D) .Rmd Line number(s) for use of regular expressions: - 


(E) .Rmd Line number(s) for use of reduction and/or transformation functions: - line 164(max)


(F) .Rmd Line number(s) for use of user-defined functions: line 83 and line 133
...
(A) .Rmd Line number(s) for use of mulitple different geoms:  geom has been used many times but i am giving one example. Line 100 and 111

(B) .Rmd Line number(s) for use of multiple aesthetics: aes has been used many times but i am giving one example. lines 100 and 111

(C) .Rmd Line number(s) for use of layered graphics: layered graphics has been used many times but i am giving one example. Line 76 and the graph after those lines. 
